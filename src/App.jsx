...
function getVerdict(ai, real, unclear) {
  ...
  if (ai / total > 0.55)
    return { label: "AI GENERATED", color: C.danger, icon: "⚡" };
  if (real / total > 0.55)
    return { label: "AUTHENTIC", color: C.accent, icon: "✓" };
...
function SubmitModal({ user, onClose, onSubmitted, onToast }) {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(false);
    if (error) {
      onToast("Submit failed: " + error.message, "error");
      return;
    }
    ...
    >
      ...
      >
        ...
        >
          ...
          <span
            onClick={onClose}
            style={{ color: C.muted, cursor: "pointer", fontSize: 18 }}
          >
            ✕
          </span>
        </div>
        ...
        />
        <input
          placeholder="Tags (comma separated: politics, deepfake)"
          value={form.tags}
...
function AuthModal({ onClose, onAuth, onToast }) {
  ...
  const handleSubmit = async () => {
    if (!form.email || !form.password) {
      onToast("Email and password required", "error");
      return;
    }
    setLoading(true);
    if (mode === "register") {
      if (!form.username.trim()) {
        onToast("Username required", "error");
        setLoading(false);
        return;
      }
      ...
      >
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          ...
          <div style={{ fontSize: 12, color: C.muted }}>
            {mode === "register"
              ? "Join the AI truth verification network"
              : "Sign in to vote and submit videos"}
          </div>
        </div>
        {mode === "register" && (
          <input
...
function ReelCard({ post, currentUser, onAuthRequired, onToast, isActive }) {
  const [votes, setVotes] = useState({
    ai: post.ai_votes || 0,
    real: post.real_votes || 0,
    unclear: post.unclear_votes || 0,
  });
    ...
    >
      ...
      <div
        style={{
          ...
          inset: 0,
          background:
            "linear-gradient(to top, rgba(0,0,0,0.92) 0%, rgba(0,0,0,0.2) 40%, rgba(0,0,0,0.1) 60%, rgba(0,0,0,0.4) 100%)",
          pointerEvents: "none",
        }}
      />
      ...
      >
        ...
        >
          ...
          >
            {initials(post.profiles?.username)}
          </div>
        </div>
        ...
        {[
          ...
          >
            ...
            <span
              style={{
                fontSize: 10,
                color: userVote === key ? color : "rgba(255,255,255,0.7)",
                fontWeight: 700,
              }}
            >
      ...
      >
        ...
        >
          <span style={{ color: avatarColor, fontSize: 13, fontWeight: 700 }}>
        ...
        >
          <div
            style={{
              width: `${(votes.ai / total) * 100}%`,
              background: C.danger,
            }}
          />
          <div
            style={{
              width: `${(votes.real / total) * 100}%`,
              background: C.accent,
            }}
          />
          <div
            style={{
              width: `${(votes.unclear / total) * 100}%`,
        ...
        >
          <span style={{ color: C.danger }}>
            ⚡ {Math.round((votes.ai / total) * 100)}% AI
          </span>
          <span style={{ color: C.accent }}>
            ✓ {Math.round((votes.real / total) * 100)}% Real
          </span>
          <span style={{ color: C.warning }}>
            ? {Math.round((votes.unclear / total) * 100)}% Unclear
          </span>
        </div>
      </div>
...
export default function App() {
  ...
  const fetchPosts = useCallback(async () => {
    ...
    if (postIds.length > 0) {
      ...
      (votesData || []).forEach((v) => {
        if (!counts[v.post_id])
          counts[v.post_id] = { ai: 0, real: 0, unclear: 0 };
  ...
  const handleWheel = useCallback(
    (e) => {
      e.preventDefault();
      if (e.deltaY > 40)
        setActiveIndex((i) => Math.min(i + 1, posts.length - 1));
            ...
            <>
              ...
              <div
                ...
                style={{
                  ...
                  fontSize: 11,
                  fontWeight: 700,
                  color: profile?.avatar_color || C.accent,
                  cursor: "pointer",
                }}
              >
          ...
          {posts.map((_, i) => (
            <div
              ...
              style={{
                ...
                borderRadius: "50%",
                background:
                  i === activeIndex ? C.accent : "rgba(255,255,255,0.3)",
                cursor: "pointer",
                transition: "all 0.2s",
